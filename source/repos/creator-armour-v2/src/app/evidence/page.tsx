...
export default function EvidenceForm() {
  ...
  const isInValidSubmission = () => !firstName || !lastName || !email || !medium || !workName || !file || !dateOfCreation || !license;
  ...
  return (
    <section className="flex flex-col-reverse md:flex-row md:justify-between h-min">
      <form onSubmit={handleSubmit} className="w-[100%] p-5 md:w-[30%] md:mt-[8rem] h-full">
        <h3 className="font-[700] text-2xl text-start md:text-4xl text-[#3A3A3A] mb-6">Evidence Form</h3>
        <section className="flex flex-col gap-4">
          ...
          <CustomInput
            ...
            placeHolder="File"            
            inputName="File"
            onChanges={(e) => setFile(e?.target?.files && e?.target?.files[0] ? e.target.files[0] : null)}
            ref={fileInputRef} 
          />
        ...
        <div className="w-[40%] mt-10">
          <PrimaryButton
            ...
            className={`${isLoading ? "bg-[#666666] px-5 py-3 cursor-wait" : "bg-primary px-5 py-3 text-white hover:bg-white hover:text-primary cursor-pointer"}`}
          />
        </div>

      </form>
      <div className="mt-[5rem] md:w-[55%] h-[15rem] md:h-[65rem]">
        <Image src={monalisa} alt={"Image of monalisa"} className="h-full w-full object-cover" width={100} height={100} />
      </div>
    </section>
  )
}
