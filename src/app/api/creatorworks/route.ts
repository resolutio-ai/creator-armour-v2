...
export async function POST(req: NextRequest) {
  try {
    ...
    const data = await creatorWorkService.createCreatorWork(
      body as ICreatorWork
    );

    return NextResponse.json(
      {
        message: "Creator Work Created",
        data,
      },
      { status: CREATED }
    );
  ...
  try {
    ...
    if (id && email) {
      return NextResponse.json(
        {
          message:
            "Only specify one query parameter to retrieve information by. Either 'id' or 'email' but never both!",
        },
        { status: BAD_REQUEST }
      );
    }
    ...
    if (id) {
      ...
      return NextResponse.json(
        {
          message: "Creator Work Retrieved",
          data,
        },
        { status: OK }
      );
    ...
    if (email) {
      ...
      return NextResponse.json(
        {
          message: "Creator Works Retrieved",
          data,
        },
        { status: OK }
      );
