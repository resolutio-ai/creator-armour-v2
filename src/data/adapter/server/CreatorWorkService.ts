...
class CreatorWorkService {
  ...
  createCreatorWork = async (model: ICreatorWork) => {
    try {
      ...
      const lightHouseResponse = await uploadEvidenceInfo({
        name: `-${model.dateOfCreation}-${model.email}-${model.workName}`,
        information: model,
      });

      const createWorkResponse = await this._repo.createCreatorWork({
        ...model,
        ipfsInfo: {
          name: lightHouseResponse.Name,
          hash: lightHouseResponse.Hash,
          size: lightHouseResponse.Size,
        },
      });
      ...
      if (isMongoError(error)) {
        throw new HttpException(
          ...
          `MongoError: ${(error as MongoError)?.message}`
        );
      }
